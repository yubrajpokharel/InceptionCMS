...
class Jadd extends CI_Controller {
	...
	function index()
	 {
	   if($this->session->userdata('logged_in'))
	   {
		 ...
		 $data = array();
			...
			$this->load->model('jadd_model');
			if ($query = $this-> jadd_model -> get_records())
			{
				$data['records'] = $query;
			}
        	
        	$this->load->view('jadd/list.php', $data);	
	 ...
	 function addnew()
	 {
	   if($this->session->userdata('logged_in'))
	   {
		 ...
		 $data = array();
        	...
        	$this->load->view('jadd/content.php', $data);	
			$this->load->view('footer.php');
	   }
	   else
	   {
...
//		}
				...
				if ( ! $this->upload->do_upload('add_pic'))
					{
						...
						$this->load->view('header.php');
						$this->load->view('jadd/content_error', $error);
						$this->load->view('footer.php');
					}
					else
					{
						...
						$this->load->model('jadd_model');
						$this->jadd_model->add_record($data);
						redirect('jadd', 'refresh');
					}
				}
				 else
			   {
		...
		function update()
		{
			...
			$this->load->model('jadd_model');
			$this->jadd_model -> update_record($data);
			redirect('jadd/show/'.$data['add_id'], 'refresh');
		}
		...
		{   
			...
			$this->load->view('header.php');
			$data = array();
			if ($query = $this->jadd_model -> get_individual_records())
			{
				$data['records'] = $query;
			}
			$this->load->view('jadd/content_show.php', $data);	
			$this->load->view('footer.php');  
		}
		...
		{   
			...
			$this->load->view('jadd/content_edit.php', $data);	
			$this->load->view('footer.php');  
		}
		...
		function change_pro_pic()
		{
			...
			if ( ! $this->upload->do_upload('pro_pic'))
				...
				else
				{
					...
					make_thumb_custom($this->upload->file_name, 'add', 200, 200);
					$this->load->model('jadd_model');
					$this->jadd_model->update_pro_pic($data);
					redirect('jadd/show/'.$data['add_id'], 'refresh');
				}
