    ...
    <!------------------------------------------------ List Portion of the CMS -------------------------------------------------------------------->        
            <div class="span9">
            <?php 
                $query = $this->db->query('SELECT * FROM jacket_advertisement where add_status = 1');
                   if ($query->num_rows() < 1){
                ?>
            <a href="<?=base_url('index.php/jadd/addnew')?>"><i class="fa fa-plus"></i> &nbsp;Create Add</a>
            <br /><br />
            <?php } ?>
                <div class="well">
                    <h3>Jacket Advertisement</h3>
                <table id="example" class="display" cellspacing="0" width="100%" style="font-size:12px;">
                 ...
                 <?php if(isset($records)) : foreach($records as $row) : ?>
                        ...
                        <tr>
							...
							<td><?php echo anchor("jadd/show/$row->add_id","$row->add_name")?></td>
                            <td>
								<?php 
